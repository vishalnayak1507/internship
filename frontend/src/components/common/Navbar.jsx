        ...
        {userRole === "superadmin" && (
          <div className="flex items-center">
            ...
            <div className="relative min-w-[180px]">
              <Listbox value={selectedDepartment || ""} onChange={handleDepartmentChange}>
                <Listbox.Button className="w-full border border-gray-300 rounded-lg py-2 pl-4 pr-10 bg-white text-gray-800 font-semibold shadow focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-150 flex items-center">
                  {selectedDepartment || "All Departments"}
                  <span className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    <ChevronDownIcon className="h-5 w-5 text-gray-400" />
                  </span>
                </Listbox.Button>
                <Listbox.Options className="absolute mt-1 w-full bg-white shadow-lg rounded-lg z-50">
                  <Listbox.Option
                    key=""
                    value=""
                    className={({ active }) =>
